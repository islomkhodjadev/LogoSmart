...
async def process_name(message: Message, state: FSMContext) -> None:
    ...
    markup.add(KeyboardButton("Xa"), KeyboardButton("Yo'q"))

    await message.answer(
        f"Salom, {message.text}!",
        reply_markup=markup,
    )
...
async def process_like_bots(message: Message, state: FSMContext) -> None:
    if message.text.lower() in ["yes", "no"]:
        ...
        await message.answer(
            "What programming language do you use?", reply_markup=ReplyKeyboardRemove()
        )
    else:
...
async def process_language(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        f"Got it! You said your name is {data['name']}, you {'like' if data['like_bots'] == 'yes' else 'don\'t like'} to write bots, and you use {data['language']}."
    )
